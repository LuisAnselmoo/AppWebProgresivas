...
function actualizarNotificacion() {
  ...
  // cantidad de mensajes no vistos
  const noVistos = Object.values(chatsActivos).filter((c) => !c.vistos).length;
  if (badgeMensajes) {
    if (noVistos > 0) {
      badgeMensajes.textContent = noVistos;
      badgeMensajes.classList.add("visible");
      badgeMensajes.classList.add("blink");
  ...
  // cantidad de notificaciones (se obtiene desde localStorage)
  ...
  // suma total = mensajes no vistos + notificaciones nuevas
